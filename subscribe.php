...
<div class="container">
    <div class="row row-flex">
        <?php           
        $select_location=mysqli_query($link,"select * from amountpay order by id desc");
        while($get_location=mysqli_fetch_array($select_location))
        {  
			$amount=$get_location['amount'];
			$name=$get_location['name'];
                        $valid=$get_location['valid'];
			$leeds=$get_location['leeds'];
        ?>
        <div class="col-xs-12 col-sm-6 col-md-4 col-lg-4">
            <div class="price-table pt-bg-green">
               <div>
                    <span><?php echo $name; ?></span>
                    <span>Price Rs.<?php echo $amount; ?></span>

                </div>
               <ul>
                 <li> Validity Period:<?php echo $valid; ?></li>
                <li> Leeds : <?php echo $leeds; ?></li>
                <li> View Profile Photos : Unlimited</li>
                <li> Search , View ,Save Profiles : Unlimited </li>
         
                </ul>
                <a href="paypa?id=<?php echo $vendorid;?>&amount=<?php echo $amount;?>&paymentmode=paidmode">purchase</a>
            </div>
        </div>
